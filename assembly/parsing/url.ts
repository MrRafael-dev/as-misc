import { SMap, SPair } from "../map";
import { WebURLInterface } from "../web";

/**
 * @class HttpURL
 * 
 * @description
 * Representa uma URL, fragmentada em vários parâmetros.
 */
 export class SiteURL implements WebURLInterface {
  protocol: string;
  username: string;
  password: string;
  href: string;
  ...
  /**
   * Cria um dicionário a partir dos parâmetros de busca de uma URL.
   *
   * @param value Parâmetros de busca (separados por `?`).
   *
   * @returns {SMap}
   */
  static takeSearchParams(value: string): SMap {
    ...
    for (let i: i32 = 0; i < params.length; i += 1) {
      ...
      result.set(key.length > 0 ? key : "_", value);
    }
   ...
   * @returns {SiteURL}
   */
  static from(raw: string): SiteURL {
    ...
    if (querySeparator >= 0) {
      ...
      for (let i: i32 = 0; i < queryStrings.length; i += 1) {
        ...
        searchParams.set(
          queryPair[0],
          queryPair[queryPair.length > 1 ? 1 : 0]
        );
      }
    }
    ...
    return new SiteURL(protocol, username, password, href, origin, host, port, pathname, search, searchParams, hash, true);
  }
}
